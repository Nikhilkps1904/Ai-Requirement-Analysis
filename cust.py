...
def call_inference_api(prompt, api_key=GEMINI_API_KEY, api_url=GEMINI_API_URL, max_retries=5, initial_delay=2):
    """Call Gemini API for inference with exponential backoff, improved error handling, and delay after 3 consecutive 429 errors"""
...
def check_new_requirement(new_req, all_existing_requirements, checked_pairs=None):
    ...
    try:
        ...
        for t in threads:
            t.join(timeout=1.0)  # Wait briefly for threads to exit
